...
The work was divided such that each of us worked on equally difficult and challenging tasks. To highlight a few contributions, Ankita led the development of the word cloud and the word vector in Shiny to identify a pattern in the reviews. Ankit worked on building the animation in Shiny that allows users to filter listings based on different criteria to give the audience a general overview of the Airbnb rental landscape in NYC. Sarang produced the animation showing the rise in the number of Airbnb rentals in NYC over the years. 

Static visualizations were also distributed such that each one of us gets to work with the entire data set and thus be able to help with issues faced by other team members and accelerate productivity.
...
listingdf1 <- read.csv('listings.csv')
...
```http://sarang-gupta.000webhostapp.com/listingAnimation.html ```
...
The data can be accessed through the below link: ```https://travel.trade.gov/view/m-2017-I-001/index.asp ```
...
sampledreviews <- read_csv('sampledreviews.csv')
...
Similarly, querying by the keyword "comfortable", we hope to see things that led to a positive experience. Prominently featured are words like "quiet", "walkable", "clean", "spotless", etc, again demostrating the importance of environment, location and cleanliness. Helpful "hosts" and "communication" lead to a comfortable. Cleanliness of linens and the bed size leave an decisive impression. 
...
library(alluvial)
library(readr)
listingdf1 <- read_csv('listings.csv')
...
Link to the animation
```https://ankitpeshin.shinyapps.io/listings/ ```
...
Link to the animation
```http://sarang-gupta.000webhostapp.com/listingGrowth.html ```

####Airbnb Word Cloud Generator

Link to the animation
```https://ankitpeshin.shinyapps.io/wordcloud_generator/ ```
